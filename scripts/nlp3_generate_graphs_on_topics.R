...
d_i_chart<-d_i_bigrams[1:10,] %>% 
  ggplot(aes(x=mentions,y=reorder(bigram,mentions))) +
  geom_bar(stat='identity',fill='#027A71')+
  ggtitle("Top topics surrounding D&I on #HR Twitter",subtitle = paste0("Tweets from ",
                                                                             as.Date(min(d_i_tweets$created_at)),
                                                                             ' to ',
                                                                             as.Date(max(d_i_tweets$created_at))))+
  xlab('Number of Mentions')+
...
#People Analytics
p_a_chart<-p_a_bigrams[1:10,] %>% 
  ggplot(aes(x=mentions,y=reorder(bigram,mentions))) +
  geom_bar(stat='identity',fill='#027A71')+
  ggtitle("Top topics surrounding People Analytics on #HR Twitter",subtitle = paste0("Tweets from ",
                                                                        as.Date(min(d_i_tweets$created_at)),
                                                                        ' to ',
                                                                        as.Date(max(d_i_tweets$created_at))))+
  xlab('Number of Mentions')+
